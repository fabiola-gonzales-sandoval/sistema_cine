...
export default function PeliculasPage() {
  ...
  const abrirModalEditar = (p: Pelicula) => {
    ...
    const data = await api<PeliculaApi[]>("/peliculas");
    setPeliculas(data.map((pelicula) => ({
      id: pelicula.id,
      ...
      imagen: IMAGENES_DISPONIBLES.find((imagen) => imagen.nombre === pelicula.titulo)?.ruta ?? "/img/banner.jpg",
      estado: "En Cartelera",
    })));
  };
  ...
  const guardarPelicula = async (e: React.FormEvent) => {
    ...
    const body = {
      ...
      fecha_estreno: form.fechaEstreno,
      sinopsis: form.sinopsis,
    };
    if (peliculaEditar) {
      await api(`/peliculas/${peliculaEditar.id}`, { method: "PUT", body: JSON.stringify(body) });
    } else {
      await api("/peliculas", { method: "POST", body: JSON.stringify(body) });
    }
    ...
    if (confirm("¿Estás seguro de eliminar esta película de la cartelera?")) {
      await api(`/peliculas/${id}`, { method: "DELETE" });
